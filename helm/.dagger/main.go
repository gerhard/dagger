...
func (h *Helm) Test(ctx context.Context) error {
	...
	err = runDaggerQuery(ctx, "dagger-dagger-helm-engine", defaultEngine)
	if err != nil {
		return err
	}

	secondEngine, err := kubectl.
		WithExec([]string{"helm", "install", "--wait", "--create-namespace", "--namespace=dagger", "--set=engine.image.ref=registry.dagger.io/engine:main", "--set=engine.kind=StatefulSet", "dagger2", "."}).
		Sync(ctx)
	if err != nil {
		return err
	}

	err = runDaggerQuery(ctx, "dagger2-dagger-helm-engine", secondEngine)
	if err != nil {
		return err
	}
...
func runDaggerQuery(ctx context.Context, engineName string, kubectl *dagger.Container) error {
